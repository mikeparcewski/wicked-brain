...
export async function runCompile(
  ...
  if (flags["dry-run"]) {
    process.stdout.write(
      `[dry-run] Would run compile agent on brain at: ${resolvedBrain}\n`
    );
    return;
  }

  const brain = await BrainHandle.open(resolvedBrain);
  await brain.lock.acquire("compile");
  const jobManager = new JobManager(path.join(resolvedBrain, "_meta"));
  const job = await jobManager.create("compile");

  try {
    process.stdout.write(`Starting compile job ${job.job_id}...\n`);

    const agent = createOperationAgent(brain, "compile");
    ...
    const prompt =
      "Compile and synthesize the knowledge in this brain. " +
      "Review all source chunks, extract key themes, and produce or update wiki articles " +
      "that synthesize the information coherently.";

    await agent.prompt(prompt);
    await agent.waitForIdle();

    await jobManager.complete(job.job_id, { messages: agent.state.messages.length });
    process.stdout.write(`Compile job ${job.job_id} completed.\n`);
  } catch (err) {
    await jobManager.fail(job.job_id, (err as Error).message);
    process.stdout.write(`Compile job ${job.job_id} failed: ${(err as Error).message}\n`);
    throw err;
  } finally {
